...
class PostnetCBHG(torch.nn.Module):
    ...
    def forward(self, x, x_lengths):
        ...
        x = x[sorted_idxs]
        x = torch.nn.utilss.rnn.pack_padded_sequence(x, sorted_lengths, batch_first=True)
        self._gru.flatten_parameters()
        x, _ = self._gru(x)
        bx, _ = torch.nn.utilss.rnn.pad_packed_sequence(x, batch_first=True, total_length=ml) 
